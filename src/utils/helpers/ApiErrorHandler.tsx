...
const SentryReporter = ({ children, message }: { children: React.ReactElement; message: string }) => {
    useEffect(() => {
...
const ErrorLayout = ({
    ...
    const content = (
        <div className={`flex flex-col items-center justify-center w-full ${isPage ? 'h-full' : 'h-70vh overflow-hidden'}`}>
...
export function NotAllowed(navigate: NavigateFunction, route: string | undefined, isPage: boolean): React.ReactElement {
    return (
        <ErrorLayout
            isPage={isPage}
            navigate={navigate}
            ...
            subtitle={isPage ? <Trans>You Are Not Allowed To Access This Page!</Trans> : <Trans>You Are Not Allowed To Access This Content!</Trans>}
        />
    )
}

export function NotFound(navigate: NavigateFunction, route: string | undefined, isPage: boolean): React.ReactElement {
    return (
...
export function SuccessEmpty(
    ...
    return (
        <ErrorLayout
            ...
            image={Empty}
            alt="no data"
            title={isPage ? <Trans>The page is not found</Trans> : <Trans>There is no data to display at the moment</Trans>}
            subtitle={isPage ? <Trans>Wrong Page Path or URL or Data is Not Found</Trans> : <Trans>data will be displayed when available</Trans>}
        />
    )
}
